...
class ExternalNodeStorage(ExternalNodeStorageBase):
    ...
    def __init__(self, nodes_uri, classes_uri, compose_node_name):
        super(ExternalNodeStorage, self).__init__(
            STORAGE_NAME, compose_node_name)

        if nodes_uri is not None:
            self._nodes_uri = nodes_uri
            self._nodes = self._enumerate_inventory(
                nodes_uri, self.node_name_mangler)
    ...
    def get_node(self, name, settings):
        ...
        p = subprocess.run(["cue", "export", "--out", "yaml", path],
                           capture_output=True)
        entity = YamlData.from_string(
            p.stdout,
            'yaml_cue://{0}'.format(path)
        ).get_entity(name, pathname, settings)
        return entity
    ...
    def get_class(self, name, environment, settings):
        ...
        try:
            ...
            raise reclass.errors.ClassNotFound(
                self.name, name, self.classes_uri)
        p = subprocess.run(["cue", "export", "--out", "yaml", path],
                           capture_output=True)
        entity = YamlData.from_string(
            p.stdout,
